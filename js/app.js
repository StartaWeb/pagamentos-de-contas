...
function renderTable(data) {
    ...
    data.forEach(bill => {
        ...
        const statusHtml = bill.status === 'paid' 
            ? `<span style="background: var(--success); color: white; padding: 4px 8px; border-radius: 4px; font-size: 0.8rem; font-weight: bold;">Pago</span>`
...
function getFilterDatesText() {
    ...
    if (filterDateStart.value && filterDateEnd.value) {
        ...
        text = `Todas as contas cadastradas`;
    }
    return text;
}
...
function exportToExcel() {
    ...
    const header = [["Relatório de Contas a Pagar"], [getFilterDatesText()], [], ["Descrição", "Cód. Barras", "Vencimento", "Valor", "Status"]];
